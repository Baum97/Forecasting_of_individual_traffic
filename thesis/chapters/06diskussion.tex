...
Ein Zweig der Forschungen, welcher eine gewisse Überschneidung mit der vorliegenden Arbeit aufweisen, beschäftigen sich mit dem \textit{Ladezustand} beziehungsweise Ladeverhalten von Elektrofahrzeugen und utilisiert dazu Fahraktivität \parencite{khwaja2021performance,kreft2024predictability,alikhani2025comprehensive,frendo2021improving,frendo2020datadriven}. Diese Arbeiten prognostizieren jedoch Ladeparameter statt des stündlichen Fahr- beziehungsweise Parkzustands, sodass weder Zielvariable noch Testprotokoll übereinstimmen und ein Kennzahlenvergleich ausscheidet.
...
\subsection*{Daten}
\begin{itemize}
    \item \textbf{Heterogene Aktivitätsdefinition:} In den Datensätzen wird die Aktivität \textit{driving} jeweils unterschiedlich definiert (siehe \Cref{sec:herausforderungen}, uneinheitliche Aktivitätssignale). Dadurch entsteht eine inhärente Heterogenität, die die Vergleichbarkeit der Ergebnisse durch potentielle Labelinkonsistenzen einschränkt.
    ...
    \item \textbf{Synthetische Quellen:} \texttt{emobpy} und \texttt{routine} bilden reales Verhalten nur unvollständig ab; bei \texttt{routine} erreicht bereits die Klimatologie-Baseline eine PR-AUC von $0,797$, sodass der zusätzliche Effekt der Modelle nur begrenzt nachweisbar ist.
    \item \textbf{Bewegung als Proxy:} In \texttt{yjmob100k} wird die Fahraktivität über das Überschreiten von Rastern abgeleitet, wodurch die Labels nur eine Annäherung an die tatsächliche Aktivität darstellen. Dies kann zu Ungenauigkeiten in der Modellbewertung und dessen Out-Of-Distribution Prognosegenauigkeit führen.
\end{itemize}

\subsection*{Experimentaldesign}
\begin{itemize}
    \item \textbf{Rekurrentes LSTM:} Diese Version des LSTM konnte auf Grund der erstellten Mindestanforderung von 100 Fahrzeugwochen nur auf \texttt{emobpy} und \texttt{yjmob100k} evaluiert werden. Eine tragende Aussage zum Architektureffekt ist daher nicht zuverlässig möglich.
    \item \textbf{Teilstichproben:} \texttt{emobpy}, \texttt{VED} und \texttt{yjmob100k} sind zu mächtig, weswegen Teilstichproben gezogen werden mussten. Die Ergebnisse gelten auf diesen Stichproben, wodurch ein Skalierungs- und Repräsentativitätsvorbehalt besteht.
    \item \textbf{Hyperparameteroptimierung:} Die Hyperparameter wurden je Modell einmalig gesetzt und als Kontrollvariable konstant gehalten. Daher geht hervor, dass nicht die absolute Bestleistung eines jeden Modells, sondern die relativen Effekte zwischen den Modellen und Datensätzen im Vordergrund stehen. 
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Klimatologie-Baseline:} Die Klimatologie erreicht bereits eine hohe Prognosegenauigkeit, sodass diese als Fallback und Pflicht-Vergleich nützt.
    \item \textbf{Wahrscheinlichkeiten:} Kalibrierung des P10/P90-Bands sorgen für eine bessere Interpretierbarkeit und liefern ein Band, statt eines Punktwertes.
...
\begin{itemize}
    ...
    \item \textbf{Vorhersagbarkeitsgrenzen:} Gedeckelter Abstand zur Baseline passt zu den theoretischen Grenzen der Vorhersagbarkeit menschlicher Mobilität, wie sie in der Literatur beschrieben werden \cite{song2010limits}.
\end{itemize}